          ...
          <div className="absolute inset-0 flex flex-col justify-between p-1 translate-y-[0%] xs:translate-y-[75%] group-hover:translate-y-0 transition-all">
            <div className='flex flex-row justify-between'>
              ...
              <div className='flex flex-row items-center justify-center gap-1 xs:gap-2 mr-1'>
                ...
                href={linkGit} 
                target='_blank' 
                className='px-2 xs:px-[16px] py-0.5 xs:py-1 bg-redPink rounded-sm'>
                  <BiGitBranch className=" text-zinc-100 text-base xs:text-xl"/></a>

                <a 
                href={linkView} 
                target='_blank' 
                className='px-2 xs:px-4 py-0.5 xs:py-1 bg-redPink rounded-sm'>
                  <BiLink className=" text-zinc-100 text-base xs:text-xl"/></a>
              </div>
            </div>

            <p className="px-1 text-left text-xs xs:text-base text-zinc-200 opacity-0 group-hover:opacity-100 transition-all">{description}</p>
          </div>

          <div className='flex flex-row top-2 left-2 absolute gap-1 opacity-0 xs:opacity-[85%] group-hover:opacity-0 transition-all'>
